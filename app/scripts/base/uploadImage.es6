(function($) {
  const AliyunUpload = require('base/aliyun-upload');
  ...
  UploadImage.prototype = {
    ...
    previewModal: function($image) {
      ...
      ZCY.utils.modal({
        ...
        html:$modal,   //自定义模板路径
        title:'添加描述',
        cls:"image-modal-preview",                                 // 自定义class
        confirm:function(m){                             //确认的callback
          const $tar = $('.image-modal-preview');
          m.modal('hide');
          $tar.remove();
      ...
      _this.options.callbacks.done = function(fileId, file) {
        ...
        AliyunUpload.getRealUrl(_this.options.downUrl, fileId)
          .then(function(result) {
            typeof _this.options.downCallback === 'function' && _this.options.downCallback(result,_this.$element);
        })
      };
      ...
      $.each(files, (index, fileId) => {
        AliyunUpload.getRealUrl(_this.options.downUrl, fileId)
          .then(function(realUrl) {
            ...
            _this.recountFile();
            typeof _this.options.downCallback === 'function' && _this.options.downCallback(realUrl,_this.$element);
          })
      })
    },
    getDownUrl: function(fid){
      let fileIds = fid || _this.$element.data('fileid');
      AliyunUpload.getRealUrl(_this.options.downUrl, fileIds)
        .then(function(result) {
        ...
        input.off('change').on('change', function(event) {
          ...
          if (window.FileReader) {
            fileOrInput = _this.filterFiles(this.files, function(file) {
              ...
              if (index >= 0) {
                let tempFile = _this.files[index]
                _this.files.splice(index, 1)
                _this.files.push(tempFile)
                _this.previewLocal(tempFile.fileId, file, fileSrc);
              } 
              return index < 0;
            });
