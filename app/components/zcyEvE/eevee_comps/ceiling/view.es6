...
const locationTemplate = Handlebars.templates["zcyEvE/eevee_comps/ceiling/templates/location"],
      optionsTemplate = Handlebars.templates["zcyEvE/eevee_comps/ceiling/templates/options"],
      siteTemplate = Handlebars.templates["zcyEvE/eevee_comps/ceiling/templates/site"]
...
class Ceiling extends OriginCeiling{
  ...
  renderSite(){
    let items = $("#eeveeSite").data("site");
    let width = items&&items.children&&items.children.length*89;
    let left = (178-width) - 99;
    let siteData = {
      items,
      width:width,
      left:left
    };
    $("#eeveeSite").append(siteTemplate(siteData));
  }
  ...
  switchRegion () {
    ...
    ZCY.utils.modal({
      button:["取消","确认"], //按钮文案
      templateUrl:"zcyEvE/eevee_comps/ceiling/templates/location",   //自定义模板路径
      title:'切换区划',
      data:{districtName: this.districtName},
      cls:"js-user-region-modal user-region-modal",                                 // 自定义class
      ...
      afterRander:function(){                 //弹出框渲染成功后的callback
        let $userRegionModal = $(".js-user-region-modal");
        $("select").selectric();
        $(".address-select", $userRegionModal).on("change", evt => that.selectAddress(evt))
  ...
  selectAddress (e, option) {
    ...
    if (!isLeaf) {
      let url = `/decorate/site/${siteid}/childsByParentCode` +
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          data = data ? data : [];
          let $select = $(
            `.js-user-region-modal .address-select[data-level=${level}]`);
          $select.html(optionsTemplate({data}));
          $select.selectric('refresh');
          $select.closest('.selectricWrapper').removeClass('selectric-hide');
        },
      });
    }
  }
  ...
  submitRegion ($form) {
    let data = $form.serializeObject(),
        ...
        $select = $(".selectricWrapper:not(.selectric-hide):last", $modal),
    ...
    if (districtCode) {
      ...
      Cookie.addCookie("districtCode", districtCode);
      Cookie.addCookie("districtName", districtName);
