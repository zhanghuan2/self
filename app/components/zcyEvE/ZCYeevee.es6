module.exports =  (function(){
  let baseConfig = require('zcyEvE/eevee_comps/config');
  ...
  Handlebars && Handlebars.registerHelper('insert2', function (a) {
    let data  = this;
    const path = arguments[0];
    let _data = arguments.length === 2 ? (arguments[1].hash||{}) :data;
  ...
  function _changeData(d){
    ...
    let cfg = $.extend({},baseConfig.defaultParam);
  ...
  function _getConfig(cfg){
    let cssJson = cfg.css ||{};
    let dataJson = cfg.data ||{};
    let result = {
      'css':_getCfgPro(cssJson),
      'data':_getCfgPro(dataJson)
    };
    return result;
  }
  function _getCfgPro(obj){
    let result = [];
    let param = $.extend({},baseConfig.defaultParam);
